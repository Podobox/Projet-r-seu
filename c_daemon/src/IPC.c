...
void send_from_c(int32_t msg_type, uint64_t posx, uint64_t posy, uint64_t type, uint64_t x) {
    ...
    message.mes.type         = type;
    message.mes.x            = x;
    printf("Sending: %i %lu %lu %lu %lu\n",
           message.mes.message_type,
           message.mes.posx, message.mes.posy, message.mes.type, message.mes.x);
...
python_struct_t *recv_from_python() {
    ...
    int length;
    if ((length = msgrcv(msqid, message, sizeof(recv_message) - sizeof(long), message->mesg_type, IPC_NOWAIT))
        < 0) {
        if (errno == ENOMSG)
            return NULL;
        perror("MSGRCV");
